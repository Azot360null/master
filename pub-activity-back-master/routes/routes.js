...
const router = app => {
    ...
    app.route("/employees")
        ...
        .post((request, response) => {
            pool.query('SELECT MAX(idEmployees) AS mx, COUNT(idEmployees) AS cnt FROM Employees', (error, result) => {
                if (error) throw error;
                let index = (result[0].cnt != 0 ? result[0].mx + 1 : 1)
        ...
        .put((request, response) => {
            pool.query('UPDATE Employees SET FIO = ?, idCathedra = ?, idSciRank = ?, idDegree = ?, idPost = ?, status = ? WHERE idEmployees = ?', [request.body.fio, request.body.idcath, request.body.idrank, request.body.iddegree, request.body.idpost, request.body.status, request.body.id], (error, result) => {
                if (error) throw error;
        ...
        .delete((request, response) => {
            ...
            pool.query('DELETE FROM Employees WHERE idEmployees = ?', [request.body.id], (error, result) => {
                if (error) throw error;
